...
def get_block_date(block_number: int, currency: str) -> pd.Timestamp:
...
def get_transaction_fee(transaction_hash: str, currency: str) -> float:
    web3: Web3 = matic_web3 if currency == "MATIC" else eth_web3

    gas_price: int = web3.eth.getTransaction(transaction_hash).gasPrice
    gas_used: int = web3.eth.getTransactionReceipt(transaction_hash).gasUsed
